/*! TBB Currency Converter — (c) thebukitbesi.com — single-owner build. License: MIT.
...
var I18N={en:{title:"💱 TBB Currency Converter — Fast, Accurate, Ad-Free",amount:"Amount",from:"From",to:"To",convert:"Convert",swap:"Swap",quick:"Quick:",options:"Options",provider:"Provider",prov_host:"Frankfurter",prov_frank:"Frankfurter",prov_auto:"Auto (Best)",cache:"Cache rates in this browser",invert:"Show inverse rate",help:"Client-side only. ISO-accurate rounding. No tracking.",copy:"Copy",share:"Share",copied:"Copied!",copyfail:"Copy failed",link:"Link copied!",converting:"Converting…",unavail:"Rate unavailable. Try other provider.",favs:"Favorites:",addfav:"Add Pair",history:"30d chart"},ms:{title:"💱 Penukar Mata Wang TBB — Pantas, Tepat, Tanpa Iklan",amount:"Amaun",from:"Dari",to:"Ke",convert:"Tukar",swap:"Tukar Arah",quick:"Pantas:",options:"Tetapan",provider:"Penyedia",prov_host:"Frankfurter",prov_frank:"Frankfurter",prov_auto:"Auto (Terbaik)",cache:"Cache kadar dalam pelayar ini",invert:"Tunjuk kadar songsang",help:"100% sisi-klien. Pembundaran ISO tepat. Tiada penjejakan.",copy:"Salin",share:"Kongsi",copied:"Disalin!",copyfail:"Salin gagal",link:"Pautan disalin!",converting:"Menukar…",unavail:"Kadar tidak tersedia. Cuba penyedia lain.",favs:"Kegemaran:",addfav:"Pin Pasangan",history:"Carta 30 hari"}};

var providers={host:{symbols:"https://api.frankfurter.dev/v2/currencies",rate:function(f,t){return "https://api.frankfurter.dev/v2/rate/"+encodeURIComponent(f)+"/"+encodeURIComponent(t)}},frank:{symbols:"https://api.frankfurter.dev/v2/currencies",rate:function(f,t){return "https://api.frankfurter.dev/v2/rate/"+encodeURIComponent(f)+"/"+encodeURIComponent(t)}}};
...
async function initWidget(root){
  ...
  async function loadSymbols(order){
    for(var i=0;i<order.length;i++){
      try{
        if(order[i]==='host'){ var h=await j(providers.host.symbols); var list=sym(h); if(list.length) return list; }
        else { var f=await j(providers.frank.symbols); var list2=sym(f); if(list2.length) return list2; }
      }catch(_){}
    }
  ...
  function sym(x){if(Array.isArray(x.value))return x.value.map(function(c){return{code:c.iso_code,name:c.name}});if(x.symbols)return Object.keys(x.symbols).map(function(c){return{code:c,name:x.symbols[c].description}});return Object.keys(x||{}).map(function(c){return{code:c,name:x[c]}})}
  function fillSelect(el,list,pref){ var a=list.filter(x=>(pref||[]).includes(x.code)); var b=list.filter(x=>!(pref||[]).includes(x.code)).sort((m,n)=>m.code.localeCompare(n.code)); el.innerHTML=[].concat(a,b).map(c=>'<option value="'+c.code+'">'+c.code+' — '+esc(c.name)+'</option>').join('') }
  ...
  async function getRate(f,t,pref){
    ...
    for(var i=0;i<order.length;i++){
      try{
        if(order[i]==='host'){ var h=await j(providers.host.rate(f,t)); var r=Number(h.rate||(h.value&&h.value[0]&&h.value[0].rate)||h.result); if(!isFinite(r)||r<=0) throw 0; var dt=(h.date||(h.value&&h.value[0]&&h.value[0].date)||'').slice(0,10); if($('.tbbx-cache',root).checked){C[key]={rate:r,date:dt,provider:'frank',ts:now}; wcache(C)}; return {rate:r,date:dt,provider:'frank',cached:false}; }
        else { var fd=await j(providers.frank.rate(f,t)); var r2=Number(fd.rate||(fd.value&&fd.value[0]&&fd.value[0].rate)||(fd.rates&&fd.rates[t])); if(!isFinite(r2)||r2<=0) throw 0; var dt2=(fd.date||(fd.value&&fd.value[0]&&fd.value[0].date)||'').slice(0,10); if($('.tbbx-cache',root).checked){C[key]={rate:r2,date:dt2,provider:'frank',ts:now}; wcache(C)}; return {rate:r2,date:dt2,provider:'frank',cached:false}; }
      }catch(_){}
    }
  ...
  async function convert(){
    ...
    try{
      ...
      out.textContent=fmt(v,f)+' '+f+' = '+fmt(res,t)+' '+t; ri.textContent='Rate: 1 '+f+' = '+precise(inv,t)+' '+t+' ('+g.provider+(g.cached?' • cached':'')+' • '+(g.date||'today')+')'; out.setAttribute('data-raw',String(res));
  ...
  async function shareRes(){ var L=I18N[getLang('en')]||I18N.en, text=$('.tbbx-out',root).textContent+' • '+$('.tbbx-rateinfo',root).textContent, url=w.location.href.split('#')[0]; try{ if(navigator.share){await navigator.share({title:'Currency Converter',text:text,url:url})} else {await navigator.clipboard.writeText(text+' '+url); toast(L.link)} }catch(_){} }
  ...
  function renderFavs(){ var wrap=$('.tbbx-favs',root); if(!wrap) return; var L=I18N[getLang('en')]||I18N.en, F=getFavs(); wrap.innerHTML='<strong class="tbbx-favs-label">'+esc(L.favs)+'</strong>'+F.map(function(k){return '<button class="tbbx-fav" data-k="'+k+'">'+esc(k)+'</button>'}).join('')+'<button class="tbbx-fav tbbx-addfav" type="button">'+esc(L.addfav)+'</button>'; $$('.tbbx-fav[data-k]',wrap).forEach(function(b){ b.addEventListener('click',function(){ var p=b.getAttribute('data-k').split('-'); $('.tbbx-from',root).value=p[0]; $('.tbbx-to',root).value=p[1]; convert() },{passive:true})}); $('.tbbx-addfav',wrap).addEventListener('click',addFav,{passive:true}); }

  function localize(root,lang){ var L=I18N[lang]||I18N.en, n; if(n=$('.tbbx-cc-title',root))n.textContent=L.title;
    ...
    if(n=$('.tbbx-convert',root))n.textContent=L.convert; if(n=$('.tbbx-swap',root))n.setAttribute('title',L.swap);
    if(n=$('option[value="host"]',root))n.textContent=L.prov_host; if(n=$('option[value="frank"]',root))n.textContent=L.prov_frank; if(n=$('option[value="auto"]',root))n.textContent=L.prov_auto;
    if(n=$('.tbbx-cache-label',root))n.textContent=L.cache; if(n=$('.tbbx-invert-label',root))n.textContent=L.invert; if(n=$('.tbbx-help',root))n.textContent=L.help;
    if(n=$('.tbbx-copy',root))n.textContent=L.copy; if(n=$('.tbbx-share',root))n.textContent=L.share; if(n=$('.tbbx-favs-label',root))n.textContent=L.favs;
    if(n=$('.tbbx-addfav',root))n.textContent=L.addfav; if(n=$('.tbbx-history-btn',root))n.textContent=L.history;
  }
