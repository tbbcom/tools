...
async function series(from,to){ var end=new Date(), start=new Date(end.getTime()-29*864e5); var s=start.toISOString().slice(0,10), e=end.toISOString().slice(0,10);
  var url="https://api.frankfurter.dev/v2/rates?from="+s+"&to="+e+"&base="+encodeURIComponent(from)+"&quotes="+encodeURIComponent(to);
  var r=await fetch(url,{mode:'cors',cache:'no-store'}); if(!r.ok) throw 0; var j=await r.json(); if(Array.isArray(j.value)) return j.value.map(function(x){return x.rate}).filter(function(x){return isFinite(x)}); var keys=Object.keys(j.rates||{}).sort(); return keys.map(k=>j.rates[k][to]); }
function attach(root){
  ...
  btn.addEventListener('click',async function(){
    btn.disabled=true; var pair=root.tbbx_pair||{}; try{
      var pts=await series(pair.from||'USD', pair.to||'MYR'); draw(cv,pts); info && (info.textContent=info.textContent.replace(/$/, " • 30d view")); 
